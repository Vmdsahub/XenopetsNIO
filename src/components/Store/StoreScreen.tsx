...
import { useGameStore, Store as GameStore, StoreItem, PurchaseResult } from '../../store/gameStore';
...
export const StoreScreen: React.FC = () => {
  ...
  const [selectedItem, setSelectedItem] = useState<{ storeItem: StoreItem; universalItem: Item | null } | null>(null);
    ...
    return (
      <div className="max-w-md mx-auto">
        ...
        >
          ...
          <div className={`bg-gradient-to-r ${getStoreTypeColor(selectedStore.type)} text-white p-6 rounded-2xl mb-4`}>
            ...
            <div className="flex items-start space-x-4">
              ...
              <div className="flex-1">
                <div className="flex items-center space-x-2 mb-2">
                  ...
                  <h3 className="font-bold text-blue-900">{selectedStore.npcName}</h3>
                </div>
    ...
    <div className="max-w-md mx-auto">
      ...
      >
        <div className="flex items-center space-x-3 mb-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Xenopets Stores</h1>
            <p className="text-gray-600">Discover amazing items for your pets</p>
          </div>
        </div>
      ...
      >
        {searchedStores.length === 0 ? (
          <div className="bg-white rounded-3xl shadow-xl p-8 border border-gray-100 text-center">
            ...
            <h3 className="text-xl font-bold text-gray-900 mb-4">No Stores Found</h3>
          ...
          searchedStores.map((store, index) => (
            ...
            >
              {/* Store Header */}
              <div className={`bg-gradient-to-r ${getStoreTypeColor(store.type)} text-white p-6`}>
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <h3 className="text-xl font-bold mb-2">{store.name}</h3>
                    <p className="text-white/90 text-sm mb-3">{store.description}</p>
                    <div className="flex items-center space-x-4 text-sm">
                      <div className="flex items-center space-x-1">
                        <Clock className="w-4 h-4" />
                        <span>{store.openHours.start}:00 - {store.openHours.end}:00</span>
                      </div>
                      <div className="flex items-center space-x-1">
                        <Package className="w-4 h-4" />
                  ...
                  <div className="text-right">
                    <div className={`w-3 h-3 rounded-full ${store.isOpen ? 'bg-green-400' : 'bg-red-400'} mb-2`}></div>
              ...
              <div className="p-6">
                ...
                <div className="flex items-center space-x-3 mb-4">
                  ...
                  <div>
                    <p className="font-semibold text-gray-900">{store.npcName}</p>
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {store.inventory.slice(0, 3).map((item) => (
                      <div key={item.id} className="bg-gray-50 rounded-lg p-2 text-center">
                        ...
                        <p className="text-xs font-medium text-gray-700 truncate">{item.itemSlug}</p>
                        <div className="flex items-center justify-center space-x-1 mt-1">
                          ...
                          <span className="text-xs font-bold">{item.currentPrice}</span>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                {store.specialOffers.length > 0 && (
                  <div className="bg-gradient-to-r from-red-50 to-pink-50 rounded-xl p-3 border border-red-200">
                    <div className="flex items-center space-x-2 mb-2">
                      ...
                      <span className="font-semibold text-red-800">Special Offers</span>
                    </div>
                    <div className="space-y-1">
                      {store.specialOffers.slice(0, 2).map((offer) => (
                        <div key={offer.id} className="flex items-center justify-between">
                          <span className="text-sm text-red-700">{offer.name}</span>
                          <span className="text-xs font-bold text-red-800">-{offer.originalPrice - offer.salePrice}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                <div className="flex items-center justify-between mt-4 pt-4 border-t border-gray-100">
                  <div className="flex items-center space-x-1">
                    ...
                    <span className="text-sm text-gray-600">Reputation: {store.reputation}</span>
                  </div>
                  <div className="flex items-center space-x-1">
                    <Star className="w-4 h-4 text-yellow-500" />
                    <span className="text-sm text-gray-600">Level {store.discountLevel}</span>
                  </div>
                </div>
              </div>
            </motion.div>
...
}> = ({ storeItem, index, onClick, getUniversalItem, getRarityColor, getItemEmoji }) => {
  ...
  if (!universalItem) {
    return (
      <div className="p-4 bg-red-50 border border-red-200 rounded-2xl">
        <p className="text-red-700 text-sm">Item not available: {storeItem.itemSlug}</p>
      </div>
    );
  }
    ...
    >
      <div className="flex items-start space-x-3 mb-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-gray-600 mb-2">{universalItem.description}</p>
          ...
          <div className="space-y-1">
            {storeItem.requirements.map((req, idx) => (
              <div key={idx} className="flex items-center space-x-1">
                ...
                <span className="text-xs text-orange-700">{req.description}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-2">
          ...
          <div className="flex items-center space-x-1">
            {storeItem.isOnSale && (
              <span className="text-sm text-gray-500 line-through">{storeItem.basePrice}</span>
            )}
        ...
        <div className="text-right">
          <div className="flex items-center space-x-1 mb-1">
            ...
            <span className="text-xs text-gray-500">Stock: {storeItem.stock}</span>
          </div>
          {storeItem.stock === 0 ? (
        ...
        {purchaseResult.success && (
          <div className="bg-green-50 rounded-xl p-3 mb-4">
            <p className="text-green-800 text-sm">
              New Balance: {purchaseResult.newBalance.toLocaleString()} {purchaseResult.currency}
            </p>
          </div>
        )}
      </div>
  ...
  return (
    <div>
      <div className="text-center mb-6">
        <div className={`w-20 h-20 mx-auto rounded-2xl border-2 ${getRarityColor(universalItem.rarity)} flex items-center justify-center mb-3 shadow-lg overflow-hidden`}>
          {universalItem.imageUrl ? (
            <img
              src={universalItem.imageUrl}
              alt={universalItem.name}
        ...
        <h3 className="text-xl font-bold text-gray-900 mb-1">{universalItem.name}</h3>
        <div className="flex items-center justify-center space-x-2">
          <span className={`px-2 py-1 rounded-full text-xs font-medium ${getRarityColor(universalItem.rarity)}`}>
            {universalItem.rarity}
          </span>
      ...
      <div className="bg-gray-50 rounded-2xl p-4 mb-6">
        <p className="text-gray-700 text-sm leading-relaxed">{universalItem.description}</p>
      </div>
      ...
      <div className="bg-blue-50 rounded-xl p-4 mb-6">
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center space-x-2">
            ...
            <span className="font-bold text-xl text-blue-900">{totalCost.toLocaleString()}</span>
