...
import { Pet, Item, Notification, User, Achievement, Collectible } from '../types/game';
...
export class GameService {
  ...
  subscribeToUserData(userId: string, callback: (data: any) => void): string {
    ...
    this.subscriptions[`${subscriptionId}_currency`] = supabase
      ...
      .on('postgres_changes', 
        { event: '*', schema: 'public', table: 'user_currency', filter: `user_id=eq.${userId}` },
  ...
  async searchPlayers(query: string): Promise<User[]> {
    try {
      const { data, error } = await supabase
        .from('profiles')
        .select('id, username, account_score, created_at, last_login, is_admin, days_played, total_xenocoins')
  ...
  async getUserAchievements(userId: string): Promise<Achievement[]> {
    try {
      ...
      return data.map(achievement => ({
        ...
        unlockedAt: achievement.user_achievements?.[0]?.unlocked_at ? new Date(achievement.user_achievements[0].unlocked_at) : undefined,
  ...
  async addUserCollectible(userId: string, collectibleName: string): Promise<boolean> {
    try {
